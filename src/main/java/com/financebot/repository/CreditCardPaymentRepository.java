...
public interface CreditCardPaymentRepository extends JpaRepository<CreditCardPayment, Long> {
    ...
    List<CreditCardPayment> findByPaymentDateBetweenOrderByPaymentDateAsc(LocalDate startInclusive, LocalDate endInclusive);

    @Query("SELECT COALESCE(SUM(p.amount), 0) FROM CreditCardPayment p WHERE p.paymentDate BETWEEN :start AND :end")
